...
public class ParticleEffectCreator : MonoBehaviour
{
    ...
    [SerializeField] private bool createMuzzleFlash = true;
    [SerializeField] private string muzzleFlashPrefabPath = "Assets/Prefabs/MuzzleFlashEffect.prefab";
    
    [Header("Destruction Effect Settings")]
    [SerializeField] private bool createDestructionEffect = true;
    [SerializeField] private string destructionEffectPrefabPath = "Assets/Prefabs/DestructionEffect.prefab";
    
    [ContextMenu("Create Particle Effects")]
    public void CreateParticleEffects()
    {
        if (createMuzzleFlash)
        {
            CreateMuzzleFlashEffect();
        }
        
        if (createDestructionEffect)
        {
    ...
    private void CreateMuzzleFlashEffect()
    {
        ...
        renderer.material = CreateMuzzleFlashMaterial();
    ...
    private void CreateDestructionEffect()
    {
        ...
        // Configure renderer
        var renderer = destruction.GetComponent<ParticleSystemRenderer>();
        renderer.material = CreateDestructionMaterial();
        
        Debug.Log("Destruction effect created successfully");
    }
    
    private Material CreateMuzzleFlashMaterial()
    {
        // Create a simple additive material for muzzle flash
        Material material = new Material(Shader.Find("Sprites/Default"));
        material.name = "MuzzleFlashMaterial";
        
        // Try to use URP additive shader if available
        Shader additiveShader = Shader.Find("Universal Render Pipeline/Particles/Unlit");
        ...
        {
            // Fallback to built-in additive
            Shader fallbackShader = Shader.Find("Mobile/Particles/Additive");
    ...
    private Material CreateDestructionMaterial()
    {
        // Create a simple material for destruction particles
        Material material = new Material(Shader.Find("Sprites/Default"));
        material.name = "DestructionMaterial";
        
        // Try to use URP particle shader if available
        ...
        {
            // Fallback to built-in particle shader
            Shader fallbackShader = Shader.Find("Mobile/Particles/Alpha Blended");
            if (fallbackShader != null)
            {
                material.shader = fallbackShader;
            }
