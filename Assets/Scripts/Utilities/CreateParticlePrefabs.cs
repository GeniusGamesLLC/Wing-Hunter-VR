...
public class CreateParticlePrefabs : MonoBehaviour
{
    ...
    private void Start()
    {
        if (createOnStart)
        {
            ...
            CreateDestructionEffectPrefab();
            
            // Destroy this GameObject after creating prefabs
            Destroy(gameObject, 1f);
        }
    }
    
    public void CreateMuzzleFlashPrefab()
    {
        ...
        main.playOnAwake = false; // Don't play on awake for prefabs
        main.loop = false;
        ...
        AutoDestroyParticleSystem autoDestroy = muzzleFlashGO.AddComponent<AutoDestroyParticleSystem>();
        autoDestroy.destroyDelay = 1f;
        
        Debug.Log("Muzzle flash prefab created in scene");
        
#if UNITY_EDITOR
        // Save as prefab in editor
        if (!Application.isPlaying)
        {
            PrefabUtility.SaveAsPrefabAsset(muzzleFlashGO, "Assets/Prefabs/MuzzleFlashEffect.prefab");
            DestroyImmediate(muzzleFlashGO);
        }
#endif
    }
    
    public void CreateDestructionEffectPrefab()
    {
        ...
        // Add auto-destroy component
        AutoDestroyParticleSystem autoDestroy = destructionGO.AddComponent<AutoDestroyParticleSystem>();
        autoDestroy.destroyDelay = 2f;
        
        Debug.Log("Destruction effect prefab created in scene");
        
#if UNITY_EDITOR
        // Save as prefab in editor
        if (!Application.isPlaying)
        {
            PrefabUtility.SaveAsPrefabAsset(destructionGO, "Assets/Prefabs/DestructionEffect.prefab");
            DestroyImmediate(destructionGO);
        }
#endif
    }
}